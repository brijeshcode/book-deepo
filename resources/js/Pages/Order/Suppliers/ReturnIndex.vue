<template>
    <app-layout title="Supplier Returns">
        ...
        <template #breadcrum>
            <bread-simple :items="[ { route: 'suppliers.index'}, {route: 'supplier.order.index', name: 'Order'}, {name:'Returns'} ]" />
        </template>
        ...
        <div v-if="showFilter" class="p-4 transition ease-in bg-white shadow rounded w-full border-red-100 my-4">
          ...
          <div class="p-4  grid  grid-cols-1  md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="filter">
              <jet-label for="supplier_order_id" required value="Supplier Order #" />
              <jet-input id="supplier_order_id" type="number" class="mt-1 block w-full" v-model="filter.supplier_order_id" autocomplete="supplier_order_id" />
            </div>

            <div class="filter">
              <jet-label for="publisher_id" required value="Supplier" />
...
s                                  </th>
                            ...
                            <Pagination :pageData="returns" pageof=" Suppliers Returns" />
                          </div>
                        </div>
                      </div>
                    </div>
...
<script>
    ...
    export default defineComponent({
        ...
        props:{
            returns: Object, suppliers: Object
        },
        data: () => ({
            showFilter: false,
            filter:{
        ...
        methods:{
          ...
          filterData(){
            Inertia.get(route('supplier.order.returns.index'), this.filter ,{
                        preserveState: true,
                        replace: true
                    });
          }
