<template>
    <app-layout title="School Order">
        ...
        <template #breadcrum>
            <bread-simple :items="[ { route: 'schools'}, {route: 'schoolOrder', name:'Orders'} , {name:'Delivery'} ]" />
        </template>
    ...
    export default defineComponent({
        ...
        computed: {
            ...
            computeTotalAmount: function () {
                let amt = 0;
                this.form.items.forEach((item) =>{ amt += parseInt(item.amount); });
                return amt;
            }*/
            ...
            if (this.order) {
                ...
                this.order.items.forEach((item, index) => {
                    ...
                    var item = {
                        ...
                        // quantity : item.quantity - item.recived_quantity,
                        quantity : 0,
                        price : 0,
                        amount: 0,
        ...
        methods:{
            submitData(){
                this.form.post(route('school.delivery.store'));
            },
            priceChange(index){
                this.form.items[index].amount = this.form.items[index].price * this.form.items[index].quantity;
            },
            amountChange(index){
                this.form.items[index].price = this.form.items[index].amount / this.form.items[index].quantity;
            }
        },
