    ...
    <app-layout title="Publishers">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Publishers
                <add-link v-if="($page.props.user.permissions.includes('create publishers'))" createRoute="publishers.create" isbutton >Add</add-link>
            </h2>
        </template>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                          <thead class="bg-gray-50">
                            <tr>
                              ...
                              <th v-if="($page.props.user.permissions.includes('edit publishers'))" scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
                            </tr>
                          </thead>
                          <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="publisher in publishers.data" :key="publisher.id">
                              ...
                              <td v-if="($page.props.user.permissions.includes('edit publishers'))" class="px-6 py-4 whitespace-nowrap text-right flex justify-end text-sm font-medium">
